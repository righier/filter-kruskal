...
public:
    ...
    kdTree(std::vector<Pos> &points): kdTree(points.begin(), points.begin(), points.end(), 0) {}
    ...
    kdTree(Iter realBegin, Iter begin, Iter end, bool dim, int depth = 0) {
        ...
        // std::cout << indent << "box: " << minp << "--" << maxp << "  node: " << pos << std::endl;

        if (mid-begin > 0) { this->left  = new kdTree(realBegin, begin, mid, !dim, depth+1); }
        if (end-mid > 1)   { this->right = new kdTree(realBegin, mid+1, end, !dim, depth+1); }
    }
    ...
    std::vector<QueryResult> closestK(const Pos &p, int id, int k) {
        ...
        // std::cout << "closest to: " << id << " " << p << " K: " << k << std::endl;
    ...
    void printDotNodes() {
        std::cout << getName() << " [pos = \"" << pos.x << "," << pos.y << "!\"];" << std::endl;
...
private:
    ...
    void closestK(const Pos &p, int id, int k, std::priority_queue<QueryResult> &best) {
