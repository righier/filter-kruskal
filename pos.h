...
static Pos operator*(const Pos &a, const float s) { return Pos(a.x*s, a.y*s); }
static Pos operator/(const Pos &a, const float s) { return a * (1.f / s); }
static Pos operator+(const Pos &a, const Pos &b) { return Pos(a.x + b.x, a.y + b.y); }
static Pos operator-(const Pos &a, const Pos &b) { return Pos(a.x - b.x, a.y - b.y); }
...
static Pos min(const Pos &a, const Pos &b) { return Pos(std::min(a.x, b.x), std::min(a.y, b.y)); }
static Pos max(const Pos &a, const Pos &b) { return Pos(std::max(a.x, b.x), std::max(a.y, b.y)); }
